...
import ini.test.IniTestRunner.TestCase;

public class IniTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".ini";
    ...
    public IniTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        IniTestSuite suite = c.getAnnotation(IniTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", IniTestSuite.class.getSimpleName(), c.getName(), IniTestRunner.class.getSimpleName()));
        }
        ...
        if (suite.testCaseDirectory() != IniTestSuite.class) {
...
// /*Not useful yet*/
//    private static final List<NodeFactory<? extends BuiltInExecutable>> builtins = new ArrayList<>();
//
//    public static void installBuiltin(NodeFactory<? extends BuiltInExecutable> builtin) {
...
//            for (NodeFactory<? extends BuiltInExecutable> builtin : builtins) {
//                IniLanguage.installBuiltin(builtin);
//            }
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            /* Parse the SL source file. */
        	 // Interactive used to be set to true, but it would display the result which is not intended
            Source source = Source.newBuilder(IniLanguage.ID, path.toFile()).interactive(false).build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        IniTestRunner suite = new IniTestRunner(testClass);
        if (args.length > 0) {
